  ...
  return (
    <main className="auth-page">
      <section className="auth-card" aria-labelledby="auth-title">
        ...
        <form autoComplete="on" onSubmit={(event) => { event.preventDefault(); mutation.mutate() }}>
          {mode === 'signup' && (
            <label>
              <span>{text.name}</span>
              <input name="name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} minLength={2} maxLength={80} required autoComplete="name" />
            </label>
          )}
          <label>
            <span>{text.email}</span>
            <input name="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="username" autoCapitalize="none" spellCheck={false} />
          </label>
          <label>
            <span>{text.password}</span>
            <input name="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={8} maxLength={128} required autoComplete={mode === 'login' ? 'current-password' : 'new-password'} />
            <small>{text.passwordHint}</small>
          </label>
          {mutation.isError && <p className="auth-error" role="alert">{errorMessage}</p>}
