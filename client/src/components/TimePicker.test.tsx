...
describe('TimePicker', () => {
  it('applies valid hour and minute values', () => {
    ...
    render(<TimePicker value="14:24" text={messages.en} onConfirm={onConfirm} />)
    ...
    expect(onConfirm).toHaveBeenCalledWith('20:05')
  })
  ...
  it('does not apply draft values when cancelled', () => {
    ...
    expect(screen.getByLabelText(messages.en.hour)).toHaveValue('14')
    expect(onConfirm).not.toHaveBeenCalled()
  })

  it('allows backspace, clearing, and retyping without moving the cursor', () => {
    const onConfirm = vi.fn()
    render(<TimePicker value="20:30" text={messages.en} onConfirm={onConfirm} />)
  ...
  it('supports arrow-key adjustment', () => {
    const onConfirm = vi.fn()
    render(<TimePicker value="23:00" text={messages.en} onConfirm={onConfirm} />)
    const hour = screen.getByLabelText(messages.en.hour)
    fireEvent.keyDown(hour, { key: 'ArrowUp' })
    expect(hour).toHaveValue('00')
  })

  it('is always visible and disables confirmation while a request is pending', () => {
    render(<TimePicker value="14:24" text={messages.en} disabled onConfirm={vi.fn()} />)
    expect(screen.getByRole('group', { name: messages.en.time })).toBeVisible()
    expect(screen.getByRole('button', { name: messages.en.ok })).toBeDisabled()
  })
})
